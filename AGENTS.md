...
- **Theme: the agency's four blues** — `#E3F2FD` ground and tinted surfaces, `#90CAF9` edges and the dark theme's text blue, `#2196F3` fills/rings/chart-1, `#0D47A1` headings and links — plus the per-module accents (`--visa`, `--hajj`, `--tour`, `--hotel`, `--expense`, `--ledger`), exposed to Tailwind as `bg-visa`, `text-hajj` and so on. (The accounts accent is `ledger`, not `balance`: `text-balance` is already a Tailwind utility for `text-wrap: balance`.) All defined as CSS variables in `globals.css`. Read the variables — never hard-code hex in a component or a chart, or dark mode breaks.
  - **`--primary` (#2196F3) fills; `--primary-strong` (#0D47A1) is for text.** White on #2196F3 is 3.12:1 — fine behind an icon or a chart bar, unreadable as a label or a link.
  - **The primary button is `.bg-gradient-primary`**, from `--gradient-primary`. Both themes define it, so no `dark:` variant is needed, and it is written once in `globals.css` — don't inline a gradient on a button.
